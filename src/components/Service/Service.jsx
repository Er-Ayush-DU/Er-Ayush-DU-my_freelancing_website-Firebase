...
const ServicesCard = () => {
  ...
  const handleCardClick = (serviceTitle) => {
    ...
    if (!currentUser) {
      navigate('/login', {
        state: { redirectAfterLogin: '/services', selectedService: serviceTitle }
      });
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceRequestRef = ref(database, `serviceRequests/${currentUser.uid}/${timestampKey}`);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h5 className="font-medium text-gray-800 mb-2">
                Service: <span className="text-blue-600">{selectedService}</span>
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                <div>
                  <p><span className="font-medium">Name:</span> {formData.name}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              whileTap="tap"
              variants={cardVariants}
              className="relative bg-white rounded-2xl overflow-hidden shadow-2xl group cursor-pointer"
          ...
          <>
            ...
            >
              <div
                className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto relative"
                style={{
                  backgroundImage: `linear-gradient(rgba(255,255,255,0.97), rgba(255,255,255,0.97)), url(${serviceImages[getImageKey(selectedService)]})`,
                  backgroundSize: 'cover',
                  backgroundPosition: 'center',
                  backgroundBlendMode: 'overlay'
                }}
                ...
                <div className="p-8">
                      ...
                      <div className="flex items-center mt-3 gap-3">
                        {[1, 2, 3].map((step) => (
                          <React.Fragment key={step}>
                            ...
                            {step < 3 && (
                              <div
                                className={`h-1 w-10 ${currentStep > step ? 'bg-blue-600' : 'bg-gray-200'}`}
                              />
                            )}
                          </React.Fragment>
                        ))}
                  ...
                  {isSubmitted ? (
                    ...
                    >
                      ...
                      <h4 className="text-2xl font-bold text-gray-800 mb-3">Request Submitted!</h4>
                      <p className="text-gray-600 mb-6 max-w-md mx-auto">
                        Thank you! Our team will contact you within 24 hours to discuss your {selectedService.toLowerCase()} project.
                      </p>
                      <button
                        onClick={handleCloseForm}
                  ...
                  {!isSubmitted && (
                    <div className="mt-8 pt-6 border-t border-gray-200">
                      <h5 className="font-medium text-gray-800 mb-3">
                        What's included in {selectedService}:
                      </h5>
                      <ul className="space-y-2">
                        {premiumFeatures[getImageKey(selectedService)]?.map((feature, i) => (
